...
def ai(messages):
    try:
        r = requests.post("https://api.groq.com/openai/v1/chat/completions",
            ...
            json={"model": "llama3-8b-8192", "messages": messages, "max_tokens": 800}, timeout=30)
...
def get_news():
    news = []
    try:
        r = requests.get("https://cryptopanic.com/api/v1/posts/?auth_token=public&kind=news&limit=10", timeout=10)
        for i in r.json().get("results", [])[:5]:
            news.append({"title": i["title"], "source": "CryptoPanic"})
    except: pass
    for url, tag in [("https://coindesk.com","h4"),("https://cointelegraph.com","h2"),("https://decrypt.co","h3")]:
        try:
            r = requests.get(url, headers={"User-Agent":"Mozilla/5.0"}, timeout=10)
            soup = BeautifulSoup(r.text, "html.parser")
            for t in soup.find_all(tag)[:2]:
                text = t.get_text().strip()
                if len(text) > 20 and not any(f in text.lower() for f in settings["filters"]):
...
def write_post(news_list):
    ...
    return ai([
        {"role": "system", "content": "Ты крипто эксперт и автор Telegram канала. Пиши интересные посты на русском. Используй эмодзи. Будь информативным."},
        {"role": "user", "content": "Напиши пост для Telegram канала: " + titles}
    ])
...
def publish(chat, post):
    ...
    send(chat, "✅ Пост опубликован в канал!")
...
def assistant(chat, text):
    ...
    system = """Ты главный AI ассистент крипто бота. Ты управляешь командой агентов:
...
Если пользователь хочет изменить настройки — объясни что напиши /settings.
